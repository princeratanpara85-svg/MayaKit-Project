      ...
      >
        {ITEMS.map((it, i) => {
          ...
          return (
            <motion.div
              key={i}
              style={{ x: tx, y: ty, z: depth, rotateX: useTransform(my, [0, 1], [10, -10]), rotateY: useTransform(mx, [0, 1], [-10, 10]) }}
              className={`absolute inset-x-4 top-${i * 12} h-28 rounded-2xl p-5 border border-white/10 bg-gradient-to-br ${it.color} shadow-2xl flex items-center gap-4`}
            >
