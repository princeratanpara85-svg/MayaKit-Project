 ...
 * AuroraMeshPattern
 * ─────────────────
 * Layered aurora "curtains" warped by a value-noise field. The mesh itself
 * is a canvas full of vertical strands whose horizontal position is
 * continuously perturbed, then composited with three colored "aurora" bands
 * that scroll across the canvas like northern lights.
 *
 * Technique: Canvas 2D, value noise (FBM) + additive blend + per-frame clear.
 * Inspired by Aceternity's "aurora background" and H5-Dooring's hero aurora
 * curtain effect, remixed with a custom strand-mesh warp.
 */
export default function AuroraMeshPattern() {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      // Three aurora "curtains" — each is a thick band of color.
      const bands = [
        { y0: 0.15, color: "rgba(56,189,248,0.55)",  hue: 195 }, // cyan
        { y0: 0.40, color: "rgba(168,85,247,0.55)",  hue: 275 }, // violet
        { y0: 0.65, color: "rgba(236,72,153,0.45)",  hue: 330 }, // pink
      ];
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute bottom-4 left-5 text-xs uppercase tracking-[0.3em] text-white/40">
        aurora · mesh · 01
      </div>
    </div>
  );
