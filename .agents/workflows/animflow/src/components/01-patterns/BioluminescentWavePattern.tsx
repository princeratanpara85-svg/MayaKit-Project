 ...
 *  Mix: Magic UI particles + 21st.dev shader grid + H5-Dooring 海浪 (wave) background. */
export default function BioluminescentWavePattern({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.fillStyle = "rgba(2,8,23,0.35)"; ctx.fillRect(0, 0, w, h);
      const step = 14;
      ...
      for (let y = 0; y < h; y += step) {
        for (let x = 0; x < w; x += step) {
          ...
          const a = (v + 1) / 2;
          const r = 1 + a * 3.5;
          ctx.fillStyle = `hsla(${180 + a * 120}, 95%, ${50 + a * 30}%, ${0.25 + a * 0.7})`;
  ...
  return <div className={cn("relative w-full h-full overflow-hidden bg-[#020617]", className)}><canvas ref={cv} className="absolute inset-0 w-full h-full" /></div>;
}
